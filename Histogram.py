...
golden_gate = cv2.imread("foto.jpg")
...
mask[1500:2000, 1000:2000] = 255 #belirtilen konum beyaz olsun
plt.figure(), plt.imshow(mask, cmap="gray")
...
plt.figure(),plt.plot(masked_img_vis, cmap="gray")
...
img_hist = cv2.calcHist([img], channels = [0], mask = None, histSize = [256], ranges = [0,256])
plt.figure(), plt.imshow(img_hist, cmap="gray")

eq_img_hist = cv2.equalizeHist(img)
plt.figure(), plt.imshow(eq_img_hist, cmap="gray")
